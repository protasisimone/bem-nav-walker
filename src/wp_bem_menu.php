...
/**
 * bem_menu returns an instance of the Walker_BEM_Menu class with the following arguments
 *
...
function bem_menu( $location = "primary", $css_class_prefix = 'menu', $css_class_modifiers = null ) {

	// Check to see if any css modifiers were supplied
	if ( $css_class_modifiers ) {

		if ( is_array( $css_class_modifiers ) ) {
			$modifiers = implode( " " . $css_class_prefix . '--', $css_class_modifiers );
		} elseif ( is_string( $css_class_modifiers ) ) {
			$modifiers = $css_class_prefix . '--' . $css_class_modifiers;
		}
	} else {
		$modifiers = '';
	}
	...
	$args = array(
		...
		'walker'         => new Walker_BEM_Menu( $css_class_prefix )
	);
